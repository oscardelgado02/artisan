...
export function openMemberEditor(nodeId: string, mid: string): void {
  ...
  el.innerHTML = `
    ...
    <textarea class="f-wide f-note" rows="2" placeholder="note for the AI (optional)">${esc(m.note ?? '')}</textarea>
    <div class="actions">
...
export function openEdgeEditor(eid: string): void {
  ...
  el.innerHTML = `
    ...
    <input class="f-wide f-enote" placeholder="note for the AI (optional)" value="${esc(e.note ?? '')}" />
    <div class="actions">
...
export function openNodeNoteEditor(nodeId: string): void {
  ...
  el.innerHTML = `
    ...
    <textarea class="f-wide f-note" rows="4" placeholder="Architecture notes for this class — the AI reads these.">${esc(n.note ?? '')}</textarea>
    <div class="actions">
...
export function openProjectNotes(): void {
  ...
  el.innerHTML = `
    <div class="p-label">Project notes — architecture rules the AI must honor</div>
    <textarea class="f-wide f-note" rows="10" placeholder="e.g. Keep the domain layer free of UnityEngine references.&#10;No god classes. Services are stateless.">${esc(state.projectNotes)}</textarea>
    <div class="actions">
